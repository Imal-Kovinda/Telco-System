...
{
    public interface IAdditionalDataPackRequestDAO
    {
        int Save(AdditionalDataPackRequest additionalDataPackRequest, DbConnection dbConnection);
        int Update(AdditionalDataPackRequest additionalDataPackRequest, DbConnection dbConnection);
        List<AdditionalDataPackRequest> GetAdditionalDataPackRequestDetailList(DbConnection dbConnection, string phoneNumber);
    }

    public class AdditionalDataPackRequestSqlDAOImpl : IAdditionalDataPackRequestDAO
    {
        public List<AdditionalDataPackRequest> GetAdditionalDataPackRequestDetailList(DbConnection dbConnection, string phoneNumber)
        {
            List<AdditionalDataPackRequest> AdditionalDataPackRequestDetailList = new List<AdditionalDataPackRequest>();
            dbConnection = new DbConnection();

            dbConnection.cmd.CommandText =
                "SELECT * FROM lb.additional_data_pack_request INNER JOIN lb.customer_account_dn ON customer_account_dn.account_dn_id = additional_data_pack_request.account_dn_id WHERE customer_account_dn.account_dn = :phoneNumber";
            ...
            AdditionalDataPackRequestDetailList = dataAccessObject.ReadCollection<AdditionalDataPackRequest>(dbConnection.dr);
            dbConnection.dr.Close();

            return AdditionalDataPackRequestDetailList;


        }

        public int Save(AdditionalDataPackRequest additionalDataPackRequest, DbConnection dbConnection)
        {
        ...
        public int Update(AdditionalDataPackRequest additionalDataPackRequest, DbConnection dbConnection)
        {
            throw new NotImplementedException();
